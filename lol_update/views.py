...
def get_more_champion_info():
    version_url = "https://ddragon.leagueoflegends.com/api/versions.json"
    version = requests.get(version_url).json()[0]
    champions = Champion.objects.values_list("name")
    for champion in champions:
        champion_url = f"https://ddragon.leagueoflegends.com/cdn/{version}/data/en_US/champion/{champion}.json"
        champion = requests.get(champion_url).json()
        # Por completar, continuar cuando sepa como llamar a los campos de la tabla
